import { addMessage, ADD_MESSAGE, popMessage, POP_MESSAGE, resetQueue, RESET_QUEUE, handleAddMessage, reducer } from "../useMessageQueue.mjs";
const EMPTY_MESSAGE = {};
...
describe("handleAddMessage", () => {
  ...
  describe("next priority", () => {
    ...
    it("should remove the old message if duplicates aren't allowed and the messageId is not the first message in the state", () => {
      const state1 = [message1];
      const next1 = { ...message1, messagePriority: "next" };
      ...
      const state2 = [message1, message2];
      const next2 = { ...message2, messagePriority: "next" };
...
describe("reducer", () => {
  const message1 = { messageId: "message-1" };
  const message2 = { messageId: "message-2" };
  const message3 = { children: "Hello, world!" };
  it("should add a new message with the correct behavior for the ADD_MESSAGE action", () => {
    expect(reducer([])).toEqual([message3]);
    expect(reducer([message3])).toEqual([message3, message3]);
  });

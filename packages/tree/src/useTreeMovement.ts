...
interface ReturnValue {
     ...
     */
    activeId: Observable<TreeItemId>
...
export function useTreeMovement({
    ...
    const expandedIds = $$(eid), selectedIds = $$(sid)
    ...
    const { activeId, onKeyDown: handleKeyDown, focusedIndex, } = useActiveDescendantMovement<SearchableTreeItem, ListElement, HTMLLIElement>({
        ...
        onChange(data) {
            ...
            const item = itemIdRefs[itemId].ref()
        ...
        onKeyDown(event) {
            ...
            switch (event.key) {
                case "Enter": {
                    ...
                    const node = itemIdRefs[itemId].ref()
                    const anchor =
                        node &&
                ...
                case "ArrowRight":
                    ...
                    if (!expandedIds.includes(itemId)) {
                        ...
                        const nextItem =
                            itemIdRefs[visibleItems[nextIndex]?.itemId]?.ref()
                ...
                case "ArrowLeft":
                    if (isParent && expandedIds.includes(itemId)) {
                        ...
                        const parentItem =
                            itemIdRefs[visibleItems[parentIndex]?.itemId]?.ref()
    ...
    const lastFocus = $(0)
    const handleBlur = $((event: JSX.TargetedFocusEvent<ListElement>) => {
        if (onBlur) {
            onBlur(event)
        }
        ...
        lastFocus(focusedIndex)
        focusedIndex(-1)
    })

    const handleFocus = $((event: JSX.TargetedFocusEvent<ListElement>) => {
        if (onFocus) {
            onFocus(event)
        }
        ...
        if (index === -1) {
            index = Math.max(0, Math.min(lastFocus(), visibleItems.length))
        }

        const currentItem = itemIdRefs[visibleItems[index]?.itemId]?.ref()
    ...
    const setActiveId = (itemId?: TreeItemId) => {

...
export interface TabsManagerProps
    extends Omit<TabsManagerContext, "activeIndex" | "onActiveIndexChange" | "tabs"> {
    /**
     ...
     * @defaultValue `0`
     */
    defaultActiveIndex?: FunctionMaybe<Nullable<number>>
     ...
     */
    activeIndex?: FunctionMaybe<Nullable<number>>
     ...
     * The list of tabs that should be controlled by the tabs manager.
     */
    tabs: readonly (TabConfig | Child | string)[]
...
export function TabsManager({
    ...
    //@ts-ignore
    children,
}: TabsManagerProps): Element {
    const localActiveIndex = $(defaultActiveIndex)
    const handleActiveIndexChange = $((activeIndex: number) => {
        if (onActiveIndexChange) {
            onActiveIndexChange(activeIndex)
        }

        localActiveIndex(activeIndex)
    })

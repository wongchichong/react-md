...
export interface ResizeListenerOptions {
   ...
   */
  options?: FunctionMaybe<Nullable<boolean>> | AddEventListenerOptions
   ...
   */
  immediate?: FunctionMaybe<Nullable<boolean>>
 ...
 */
export function useResizeListener({ onResize, options, immediate, enabled = true, }: ResizeListenerOptions): void {
  ...
  useEffect(() => {
    ...
    const handler = (event: Event): void => callback()(event)

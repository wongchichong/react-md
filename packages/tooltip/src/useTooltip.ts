...
export interface TooltipPositioningOptions {
     ...
     * between the container element and the tooltip.
     */
    denseSpacing?: FunctionMaybe<Nullable<number>> | string
     ...
     */
    disableAutoSpacing?: FunctionMaybe<Nullable<boolean>>
}
...
export interface BaseTooltipHookOptions<E extends HTMLElement>
     ...
     */
    touchTime?: FunctionMaybe<Nullable<number>>
     ...
     */
    focusTime?: FunctionMaybe<Nullable<number>>
     ...
     */
    disableHoverMode?: FunctionMaybe<Nullable<boolean>>
     ...
     * Real World Example
     * ```tsx
     * import type { ReactElement, Child } from "react";
...
export interface TooltipHookOptions<E extends HTMLElement>
     ...
     */
    describedBy?: FunctionMaybe<Nullable<string>>
}
...
export type TooltipHookProvidedTooltipProps = Pick<TooltipProps, "style"> &
    ...
    Required<Pick<TooltipProps, "id" | "dense" | "position" | "visible">> & {
        ref: ObservableMaybe<HTMLSpanElement>
    }
...
export function useTooltip<E extends HTMLElement>({
    ...
    const containerRef = $<E | null>(null)
    ...
    const initiatedBy = $<TooltipInitiatedBy>(null)
    const windowFocusEvent = $(false)
    ...
    const onBlur = (event: JSX.TargetedFocusEvent<E>): void => {
        //@ts-ignore
        propOnBlur?.(event)
    ...
    const onFocus = (event: JSX.TargetedFocusEvent<E>): void => {
        //@ts-ignore
        propOnFocus?.(event)
        if (disabled) {
            return
        ...
        initiatedBy("keyboard")
        window.clearTimeout(timeout())
        //@ts-ignore
        containerRef(event.currentTarget)
        //@ts-ignore
    ...
    const onKeyDown = (event: JSX.TargetedKeyboardEvent<E>): void => {
        //@ts-ignore
        propOnKeyDown?.(event)
    ...
    const onTouchStart = (event: JSX.TargetedTouchEvent<E>): void => {
        //@ts-ignore
    ...
    const onContextMenu = (event: JSX.TargetedMouseEvent<E>): void => {
        //@ts-ignore
    ...
    useEffect(() => {
        ...
        const handler = (): void => {
            if (document.hidden) {
                hide()
                windowFocusEvent(document.activeElement === containerRef())
            }
        }
    ...
    const nodeRef = $<HTMLSpanElement>(null)
    const {
        ref,
        style,
    ...
    const elementProps: TooltipHookProvidedElementProps<E> = {
        id: baseId,
        //@ts-ignore
        "aria-describedby": [visible && tooltipId, describedBy] || undefined,
        ...tooltipHandlers,
    }
